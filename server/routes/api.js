...
router.get("/quizzes", async (req, res) => {
  try {
    const quizzes = await models.Quiz.find()
...
router.get("/quizzes/:slug", async (req, res) => {
  try {
    const quiz = await models.Quiz.findOne({ slug: req.params.slug }).populate(
      "questions"
    ); // This ensures questions are fully loaded
...
router.post("/quizzes", async (req, res) => {
  try {
    ...
    const existing = await models.Quiz.findOne({ slug: generatedSlug });
    if (existing) {
      return res.status(409).json({ error: "Slug already in use" });
    }

    // Create questions
    const createdQuestions = await models.Question.insertMany(
      questions.map((q) => ({
        id: q.id,
        type: q.type,
        question: q.question,
